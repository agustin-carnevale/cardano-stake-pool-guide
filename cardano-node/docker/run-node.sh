...
## The folder, on the host machine Pi where to download the blockchain
DB_FOLDER=$1
...
if [ "${NODE_MODE}" = "relay" ]; then
  ...
  docker run --name "cardano-node-${NETWORK}" -d -v $DB_FOLDER:/db -e CARDANO_NODE_SOCKET_PATH=/db/node.socket \
    -p "${CARDANO_NODE_PORT}:${CARDANO_NODE_PORT}" -p 12798:12798 -v /home/ubuntu/my-cardano-staking-pool/cardano-node/config/mainnet:/etc/config/cardano-node/config \
  ...
  docker run --name "cardano-node-${NETWORK}" -d -v $DB_FOLDER:/db -e CARDANO_NODE_SOCKET_PATH=/db/node.socket \
    -v /home/ubuntu/my-cardano-staking-pool/cardano-node/config/mainnet:/etc/config/cardano-node/config \
    -v /home/ubuntu/.keys/mainnet:/home/cardano/keys \
